...
<p align="center">
  <img src="screenshots/List%20Static%20Tree%20Indentation%20Guides%20-%20List%20Tree%20Indentation%20Guides%20Plugin.png"
       alt="List Static Tree Indentation Guides">
  <br>
  <em>List Static Tree Indentation Guides</em>
</p>
...
**Thread connector height** defaults to **103%** in the main view; the independent breadcrumb control defaults to **100%**. Threads start from the parent marker, using the visible bullet, numeral text range, or checkbox bounds. Wrapped parent text does not shorten the connector. At 100%, the upper end leaves the configured marker gap and stroke-cap clearance. Lower percentages shorten the upper end of each elbow; they do not move the child's horizontal connector. Values above 100% reduce that gap, stopping when the stroke reaches the parent marker boundary. Increasing the value further cannot draw through or above the marker, including when a distant later child is active. The slider reaches 500%; its precise value field accepts any nonnegative finite percentage, including larger values. Breadcrumb threading has its own height control.

**Thread marker gap** defaults to **6.5 px** in the main view and **4 px** in breadcrumbs. Saved values take precedence. Numbered tasks reserve space for both the number and the checkbox. At the default **28 px** Thread connector length, a child branch aligns below its parent's primary marker (the numeral for numbered tasks, or the checkbox for unnumbered tasks). Increasing the length moves that spine outward; changing the child's marker type or marker gap no longer moves the parent attachment. Root branches without a marked parent retain their fixed horizontal reach.
