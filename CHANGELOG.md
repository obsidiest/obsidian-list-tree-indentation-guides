...
- Add an independent **Thread separate list blocks that are only separated by a blank line** toggle beneath **Active List Item Threading**, disabled by default, while retaining the independently controlled All Branches version.
- Add enabled-by-default **Active Orphan List Threading** with enabled-by-default **Active Orphan List Item Threading** and disabled-by-default **All Branches of an Active Orphan List Threading** for ordered and unordered list blocks without unmarked heads.
- Add root-level orphan paths in Live Preview, Source mode, and Reading mode, including a horizontal thread for a single hovered root item and complete root sibling groups for All Branches.
- Restrict inferred off-screen continuation/gutter spines to the explicit **Connect separate list blocks** opt-in, preventing the intermittent far-right overdraw when that setting is disabled.
- Standardize **List Static Tree Indentation Guides**, **List Threading**, and “Thread separate list blocks” terminology throughout settings, Style Settings, and documentation.
- Preserve 1.0.5 settings through one-time legacy-key normalization, searchable definitions for every current option, precise numeric fields, viewport-only rendering, and attested release assets.
...
- Rename **All Branches of an Active List Threading** for clearer ordered/unordered-list coverage.
...
- Add **Thread separate list blocks that are only separated by a blank line**, disabled by default, as an all-branches-only opt-in; nonblank content remains a hard threading boundary.
- Add **List threading from a non-bulleted/numbered list head**, enabled by default, for Active List Item and All Branches threading in editor and Reading modes.
- Add the global **Enable list static tree indentation guides** toggle above the three mode toggles under the new **List Static Tree Indentation Guides** section.
...
- Add enabled-by-default Style Settings toggles for all eight list-thread colors, plus enabled-by-default themed fallback colors and disabled-by-default themed override colors.
- Rename **Guide appearance** to **List Static Tree Indentation Guide Appearance** and retain synchronized precise numeric fields for every numerical Style Settings control.
...
- Make editor list threading respond to the full vertical hit area of every visible list-item row instead of depending on which child DOM element receives the pointer event.
- Add **Active List Item Threading**, enabled by default, and **All Branches of an Active List Threading**, disabled by default.
...
- Derive list-thread hover targets and ancestor paths from the same rendered rows, fixing inactive threading when the parser model was empty or delayed.
...
- Add optional Logseq-style list threading with a global toggle and independent Live Preview, Source, and Reading-mode toggles.
